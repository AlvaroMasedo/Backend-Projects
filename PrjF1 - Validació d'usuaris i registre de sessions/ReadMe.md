...
### 7. Fitxer .htaccess (Seguretat)

**Descripció:** Fitxer de configuració d'Apache que implementa mesures de seguridad bàsiques per a protegir l'aplicació.

**Què fa:**
- **Deshabilita llistat de directoris:** Evita que els visitants vegin el contingut de les carpetes públiques.
- **Bloqueja arxius sensibles:** Impedeix l'accés directe a arxius de configuració (`.env`, `.sql`, `.config`, `.htaccess`).
- **Deshabilita PHP en uploads:** Evita que els usuaris subin i executin scripts maliciosos (PHP, exec, etc.) a la carpeta `uploads/`.
- **Headers de seguretat:** Afegeix capçaleres HTTP que protegeixen contra atacs comuns:
  - `X-Frame-Options: SAMEORIGIN` — Evita "click-jacking" (no permet embed en iframes d'altres dominis).
  - `X-Content-Type-Options: nosniff` — Prevé "MIME sniffing" (evita que el navegador reinterprete arxius).
  - `X-XSS-Protection: 1; mode=block` — Habilita protecció contra "XSS" (cross-site scripting).

**Ubicació:** Arrel del projecte (`/.htaccess`).

**Per què és important:**
- Redueix exponencialment les vulnerabilitats comunes.
- Protegeix la configuració sensible (credencials BD, etc.).
- Limita l'accés a arxius de codi font.
- Millora la seguretat contra atacs automatitzats.