...
function iniciarSessio()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($email) || empty($contrasenya)) {
            ...
            // Obtenir l'usuari per verificar si és OAuth
            $usuari = $controlarUsers->obtenirPerEmail($email);

            // Comprovar si el compte és OAuth (contrasenya = 'oauth_pending')
            if ($usuari && $usuari['contrasenya'] === 'oauth_pending') {
